...
namespace Kiwi
{
    struct Initializer
    {
        ...
        Initializer() :
        instance(nullptr), patcher(nullptr), lid(0), name(""), text(""), dico(nullptr), args({})
        {
            ;
        }
        
        Initializer(sInstance _instance, sPatcher _patcher, const ulong _id, const string _name, const string _text, scDico _dico, ElemVector const& _args) :

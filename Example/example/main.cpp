    ...
    if(manager)
    {
        ...
        if(instance)
        {
            ...
            if(page)
            {
                ...
                page->add(Dico::evaluateForObject("noise~ @position 20 10"));
                page->add(Dico::evaluateForObject("dac~ 1 2 @position 30 10"));
                
                page->add(Dico::evaluateForLink("2 0 3 0"));
                page->add(Dico::evaluateForLink("2 0 3 1"));
                
                vector<sObject> objs;
                page->getObjects(objs);
                cout << "Number of objects : "<< objs.size() << endl << endl;
                for(vector<sObject>::size_type i = 0; i < objs.size(); i++)
                {
                    cout << "Object : " << toString(objs[i]->getName()) << endl;
